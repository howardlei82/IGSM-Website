<?php
/**
 * The template used to display the footer for Blank template
 *
...
?>
    ...
    <?php ttw_put_area('prefooter'); ?>
    <?php
    if (!ttw_getopt('ttw_hide_footer')) {
    ?>
	<div id="footer">
		<div id="colophon">
...
<?php $cp = ttw_getadminopt('ttw_copyright');
	if (strlen($cp) > 0) { echo(str_replace("\\", "", ttw_getadminopt('ttw_copyright'))); echo('</div></td>'); }
	else { ?>
 ...
 <a href="http://wordpress.org/" title="<?php esc_attr_e( 'Semantic Personal Publishing Platform', TTW_TRANS ); ?>" rel="generator"><?php printf( __( 'Proudly powered by %s.', TTW_TRANS ), 'WordPress' ); ?></a>
 <?php echo(TTW_THEMENAME); ?> by WPWeaver.info
    ...
    <?php } ?>
    <?php ttw_put_area('postfooter'); ?>
    <?php ttw_browser_warning(); ?>
